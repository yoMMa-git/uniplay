      ...
      {invites.length>0 ? invites.map(inv=>(
        <Card key={inv.id}>
          <CardContent className="flex justify-between items-center">
            <div>
              Invite to <strong>{inv.team.name}</strong> от <em>{inv.inviter.username}</em>
            </div>
            <div className="space-x-2">
              <Button size="sm" onClick={()=>handle(inv.id,'accept')}>Принять</Button>
              <Button size="sm" variant="destructive" onClick={()=>handle(inv.id,'decline')}>Отклонить</Button>
            </div>
          </CardContent>
        </Card>
