...
interface Profile { id: number; role?: string; }
interface Team { id: number; name: string; game: Game; members: User[]; captain: User; }
...
export default function TeamsList() {
  ...
  const members = teams.filter(t => t.captain.id !== profile.id && t.members.some(m => m.id === profile.id));
  const displayed = filter === 'all' ? teams : filter === 'owned' ? owned : members;
            ...
            <Dialog open={isOpen} onOpenChange={setIsOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>Введите название и выберите дисциплину</DialogDescription>
                </DialogHeader>
                <form onSubmit={handleCreate} className="space-y-4">
                  <Input placeholder="Название команды" value={newName} onChange={e => setNewName(e.target.value)} required />
      ...
      {loadingTeams ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {displayed.map(team => {
            ...
            return (
              <Link to={`/teams/${team.id}`} key={team.id} className="block hover:shadow-lg transition">
                <Card>
                  <CardContent className="flex justify-between items-start">
                    <div>
                      <h2 className="text-lg font-semibold">{team.name}</h2>
                      <p className="text-sm text-gray-600">Дисциплина: {team.game.name}</p>
                      <p className="text-sm text-gray-600">Участников: {team.members.length}</p>
                    </div>
